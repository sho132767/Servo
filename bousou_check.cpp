...
int main(){
    ...
    for(int i = 0; i < 4; i++){
        pid[i].set_limit(c.MAX_CURRENT, c.MIN_CURRENT);
    }
    ...
    bno.get_theta();
    s.theta = bno.theta - 2 * M_PI;
    s.mid_theta = s.theta_prev + atan2(sin(s.theta - s.theta_prev), cos(s.theta - s.theta_prev)) / 2.0;
        ...
        s.measured_vx = s.l_vx * cos(s.mid_theta) - s.l_vy * sin(s.mid_theta);
        s.measured_vy = s.l_vx * sin(s.mid_theta) + s.l_vy * cos(s.mid_theta);
        ...
        // pc.printf("bno%lf rob%lf", s.bno_omega, s.robot_omega);
    ...
    // pc.printf(" %d %d %d %d",s.current[0],s.current[1],s.current[2],s.current[3]);
        ...
        */
            ...
            // 2. 角度がズレているときは位置修正を弱める（干渉防止）
            if (fabs(s.angle_error) > 0.1) { // 約5度以上ズレたら
                s.desired_vx *= 0.2; // 位置修正を20%に抑える
                s.desired_vy *= 0.2;
            }
            ...
            //     s.target_omega = 0.0;
            //     s.theta_integral = 0.0;
            // } else {
            //     s.theta_integral += s.theta_error[0] * c.dt;
            //     if(fabs(s.theta_integral) > c.vmax_ang) s.theta_integral = c.vmax_ang * (s.theta_integral > 0 ? 1 : -1);
            ...
            //符号による低速制御
            if (fabs(s.angle_error) > c.ang_tol) {
                s.target_omega = (s.angle_error > 0 ? 0.5 : -0.5); // 0.5rad/s で定速回転
            } else {
                s.target_omega = 0;
            }
        ...
        //  もともとのtar[]計算
            ...
            double cmd_l_vx =  s.vx * cos(s.theta) + s.vy * sin(s.theta);
            double cmd_l_vy = -s.vx * sin(s.theta) + s.vy * cos(s.theta);


            s.tar[0] = (cmd_l_vx * -sin(M_PI / 4) + cmd_l_vy * cos(M_PI / 4) + s.omega * c.R) / c.r;
            s.tar[1] = (cmd_l_vx * -sin(3 * M_PI / 4) + cmd_l_vy * cos(3 * M_PI / 4) + s.omega * c.R) / c.r;
            s.tar[2] = (cmd_l_vx * -sin(5 * M_PI / 4) + cmd_l_vy * cos(5 * M_PI / 4) + s.omega * c.R) / c.r;
            s.tar[3] = (cmd_l_vx * -sin(7 * M_PI / 4) + cmd_l_vy * cos(7 * M_PI / 4) + s.omega * c.R) / c.r;
